...
const kGrpcContentTypeHeader = 'content-type';
const kGrpcContentType = 'application/grpc';
...
class ServerCall extends EventEmitter {
  ...
  receiveMetadata(headers) {
    ...
    const contentTypeHeader = metadata.get(kGrpcContentTypeHeader);
    if (contentTypeHeader.length > 0) {
      const contentType = contentTypeHeader[0];
      if (contentType !== kGrpcContentType) {
        this.stream.respond({ ':status': 415 }, { endStream: true });
      }
    }
